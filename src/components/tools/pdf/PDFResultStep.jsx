...
const PDFResultStep = ({ data, updateData, goBack, setResult, result, themeColor }) => {
   ...
   if (!result && data.files && data.options) {
     const processData = async () => {
       ...
       try {
         ...
         // Simuliamo un risultato di successo per test
         if (!processedResult) {
           console.log("API non implementata, simulo risultato");
           processedResult = {
             success: true,
             type: 'pdf',
             blob: new Blob([await new Response(data.files[0]).arrayBuffer()], { type: 'application/pdf' }),
             filename: `processed_${data.files[0].name}`,
 ...
 }, [data.files, data.options, data.conversionType, result, setResult]);
